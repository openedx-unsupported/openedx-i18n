...
def pull_translations():
    for config in xblock_configs():
        ...
        call(['git', 'checkout', '--branch', branch], cwd=repo_dir)
        ...
        call(['git', 'config', 'user.name', 'Open edX i18n Bot'], cwd=repo_dir)
        call(['git', 'config', 'user.email', 'omar+openedx-i18n-bot@appsembler.com'],
             cwd=repo_dir)

        call(['git', 'add', '--all'], cwd=repo_dir)
        call(['git', 'commit', '-m', 'Update translations (autogenerated message)'],
             cwd=repo_dir)

        call(['git', 'branch', '--set-upstream', branch,
              'local/{branch}'.format(branch=branch)], cwd=repo_dir)
