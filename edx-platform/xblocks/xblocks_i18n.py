...
def pull_translations():
    for config in xblock_configs():
        ...
        try:
            ...
            check_output(['git', 'clone', config['upstream_repo'], config['name']], cwd=repos_dir)
            check_output(['git', 'remote', 'add', 'local', config['local_repo']], cwd=repo_dir)
            check_output(['git', 'checkout', '-b', branch, 'master'], cwd=repo_dir)
            check_output(config['requirements_script'], shell=True, cwd=repo_dir)
        ...
        try:
            check_output(config['pull_script'], shell=True, cwd=repo_dir)
        except Exception as e:
            yield {
                'name': config['name'],
                'stage': 'pull',
        ...
        try:
            check_output(['git', 'config', 'user.name', 'Omar Al-Ithawi'], cwd=repo_dir)
            check_output(['git', 'config', 'user.email', 'i@omardo.com'], cwd=repo_dir)
            check_output("git add $(find . -name '*.po' -or -name '*.mo' -or -name '*.js')", shell=True, cwd=repo_dir)
            check_output(['git', 'commit', '-m', 'Update translations (autogenerated message)'], cwd=repo_dir)
        except Exception as e:
            yield {
                'name': config['name'],
                'stage': 'commit',
